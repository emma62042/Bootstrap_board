...
class PostModifyView {//新增修改留言的子類
    function __construct(&$msg_array, $title) {
        if(isset($msg_array["success"]) && $msg_array["success"] == 1){
            $type = $title=="post" ? "新增" : "修改";
            echo "<script>alert('". $type . "成功!');window.location.href='index.php';</script>";
        }else{?>
...
class DeleteView {
    function __construct (&$value) {
        if(isset($value["success"]) && $value["success"] == 1){
            echo "<script>alert('刪除留言成功!');history.back();</script>";
        }else{
            if(isset($_GET["id"])){
            ?>
                <form action="index.php?action=delete" method="post">
...
class SignupView {
    function __construct (&$signup_array) {
        if(isset($signup_array["success"]) && $signup_array["success"] == 1){
            echo "<script>alert('註冊成功!!跳轉至登入頁面');window.location.href='index.php?action=login';</script>";
        }else{?>
        		<h2>會員註冊</h2>
...
class ModifyMyDataView {
    function __construct(&$mb_array) {
        if(isset($mb_array["success"]) && $mb_array["success"] == 1){
            echo "<script>alert('修改成功!!跳轉至首頁');window.location.href='index.php';</script>";
        }else{?>
        	<h2>會員專區</h2>
...
class ModifyMyPwdView {
    function __construct(&$mb_array) {
        if(isset($mb_array["success"]) && $mb_array["success"] == 1){
            echo "<script>alert('修改密碼成功!!跳轉至首頁');window.location.href='index.php';</script>";
        }else{?>
        	<h2>會員專區</h2>
